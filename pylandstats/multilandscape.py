...
class MultiLandscape:
    ...
    def plot_metric(self, metric, class_val=None, ax=None, metric_legend=True,
                    method='plot', method_kws={}, fmt='--o', subplots_kws={}):
        """
        Parameters
        ----------
        metric : str
        ...
        if method == 'plot':
            ax.plot(feature_values, metric_values, fmt, **method_kws)
        else:  # method == 'bar'
            x = range(len(feature_values))
            ax.bar(x, metric_values, **method_kws)
            ax.set_xticks(x)
    ...
    def plot_metrics(self, class_val=None, metrics=None, num_cols=3,
                     metric_legend=True, xtick_labelbottom=False,
                     method='plot', method_kws={}, fmt='--o',
                     subplots_adjust_kws={}):
        """
        Parameters
        ----------
        ...
        for metric, ax in zip(metrics, flat_axes):
            self.plot_metric(metric, class_val=class_val, ax=ax,
                             metric_legend=metric_legend, method=method,
                             method_kws=method_kws, fmt=fmt)
